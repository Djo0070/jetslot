...
def init_firestore():
    ...
    if firebase_cred_str:
        try:
            ...
            # Si la chaîne contient des \n littéraux, les remplacer par de vrais retours à la ligne
            if '\\n' in firebase_cred_str:
                firebase_cred_str = firebase_cred_str.replace('\\n', '\n')
            
            # Supprimer les guillemets simples superflus
            if firebase_cred_str.startswith("'") and firebase_cred_str.endswith("'"):
                firebase_cred_str = firebase_cred_str[1:-1]
            
            # Supprimer les guillemets doubles superflus au début et à la fin
            if firebase_cred_str.startswith('"') and firebase_cred_str.endswith('"'):
                firebase_cred_str = firebase_cred_str[1:-1]
            ...
            st.warning(f"Erreur de parsing JSON : {e}")
            ...
            try:
                ...
                st.warning(f"Erreur de réparation : {e2}")
        except Exception as e:
